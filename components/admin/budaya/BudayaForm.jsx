...
export default function BudayaForm({ initialData, onSubmit, loading }) {
  const [uploadMode, setUploadMode] = useState('upload');
  const [formData, setFormData] = useState({ judul: '', deskripsi: '', isi: '', imagelink: '' });
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        ...
        isi: initialData.isi || '',
        imagelink: initialData.imagelink || initialData.imageLink || ''
      });
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => {
        document.getElementById('modal_budaya').close();
        setStatus({ type: '', message: '' });
      }, 1500);
    } catch (err) {
  ...
  return (
    <div className="w-full text-base-content">
      <div className="bg-primary/10 p-3 rounded-lg mb-5 border-l-4 border-primary">
        <h4 className="text-xs font-bold text-primary uppercase tracking-wider">Form Input Budaya</h4>
      ...
      <form onSubmit={handleFormSubmit} className="relative">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-7 space-y-4">
            <div className="form-control w-full">
              <label className="label-text font-bold text-xs mb-1">Judul Budaya <span className="text-error">*</span></label>
              <input className="input input-bordered input-sm w-full focus:input-primary transition-all" placeholder="Nama Kebudayaan..."
                value={formData.judul ?? ""} onChange={(e) => setFormData({ ...formData, judul: e.target.value })} required
              />
            </div>

            <div className="form-control w-full">
              <label className="label-text font-bold text-xs mb-1">Deskripsi Singkat</label>
            ...
            <div className="form-control w-full">
              <label className="label-text font-bold text-xs mb-1">Isi Konten Lengkap</label>
        ...
        <div className="flex justify-end gap-3 mt-8 pt-4 border-t border-base-200">
          <button type="button" onClick={() => document.getElementById('modal_budaya').close()} className="btn btn-ghost btn-sm px-6">Batal</button>
